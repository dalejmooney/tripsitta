...
<div class="invoice-box">
    <table cellpadding="0" cellspacing="0">
                        ...
                        <td>
                            ...
                            @if($invoice->babysitter_address !== 'null' && is_array($invoice->babysitter_address))
                                ...
                                @if($invoice->babysitter_address['country'] !== '')
                                    {{Countries::getOne($invoice->babysitter_address['country'])}}<br>
                                @endif
                            @endif
                        </td>
                    </tr>
            ...
            <td colspan="2">
                <table>
                    <tr>
                        <td>
                        Parent:
                        <strong class="is-bold">{{$invoice->family->user->fullName}}</strong><br>
                        @if($invoice->family_address !== 'null' && is_array($invoice->family_address))
                            {{$invoice->family_address['address1']}},
                            @if($invoice->family_address['address2'] !== '')
                                {{$invoice->family_address['address2']}},
                            @endif
                            {{$invoice->family_address['town']}}, {{$invoice->family_address['postcode']}},
                            @if($invoice->babysitter_address['country'] !== '')
                                {{Countries::getOne($invoice->family_address['country'])}}<br>
                            @endif
                        @endif
                {{$invoice->family->user->email}}</tr>
                </table>
            </td>
        </tr>
        ...
        <tr class="item last">
            <td>
                <span class="capitalize">Paid to babysitter</span>
            </td>
            <td>
                - {!! $invoice->babysitterEarningFormatted !!}
            </td>
        </tr>
        ...
        <tr class="total">
            <td></td>
            <td>
                Total: {!! $invoice->adminEarningFormatted !!}
            </td>
        </tr>

        <tr>
            <td colspan="2" style="font-size:12px; text-align: center; padding-top:50px;">
