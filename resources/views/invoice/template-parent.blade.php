...
{{--                            @endif--}}
                        ...
                        <td>
                            {{$invoice->family->user->fullName}}<br>
                            @if($invoice->family_address !== 'null' && is_array($invoice->family_address))
                                {{$invoice->family_address['address1']}}<br>
                                @if($invoice->family_address['address2'] !== '')
                                    {{$invoice->family_address['address2']}} <br />
                                @endif
                                {{$invoice->family_address['town']}}, {{$invoice->family_address['postcode']}}<br>
                                @if($invoice->babysitter_address['country'] !== '')
                                    {{Countries::getOne($invoice->family_address['country'])}}<br>
                                @endif
                            @endif
                            {{$invoice->family->user->email}}
                        </td>
                    </tr>
                </table>
            </td>
        ...
        <tr class="total">
            <td></td>
            <td>
                Total: {!! $invoice->amountDueFormatted !!}
            </td>
        </tr>
