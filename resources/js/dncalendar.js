 ...
 (function ( $ ) {
            ...
            for (var j = 0; j < nDate; j++) {
              ...
              if (typeof settings.notes !== 'undefined') {
                if (dateIsNotes(new Date(year, month - 1, date))) {
                    let note_types = getNote(new Date(year, month - 1, date)).note.join(' ');
    ...
    var getNotesThisMonth = function() {
      ...
      update: function(options) {
            ...
            addNoteExpanded: function(date) {
                ...
                if(added !== true && removed !== true)
                {
                    tripsitta_notes.push(
                        { "date": dateString, "note": ['babysitter-day', 'babysitter-night'] }
                    );
                }
            ...
            addNote: function(date, new_note) {
                var tripsitta_notes = this.returnNotes();
                var dateString = new Date(date.getTime() - (date.getTimezoneOffset() * 60000 ))
                    .toISOString()
                    .split("T")[0];
