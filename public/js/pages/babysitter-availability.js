  ...
  $.fn.dnCalendar = function (options) {
                      ...
                      if (!isPush) {
                        ...
                        if (typeof tempArr[tempArr.length - 1] !== 'undefined') {
                          ...
                          if (lastIndexTempArr + 1 != val && lastIndexTempArr + 1 < lastDate) {
                            ...
                            tempArr.push(weeks[newI][j + startDayWeek]); // console.log("first col ["+ newI +"]["+ (j + startDayWeek) +"] => " + val);
                          }
                        } else {
                          tempArr.push(weeks[newI][j + startDayWeek]); // console.log("first col ["+ newI +"]["+ (j + startDayWeek) +"] => " + val);
                    ...
                    if (typeof weeks[newI + 1] !== 'undefined') {
                      for (var k = 0; k < weeks[newI + 1].length; k++) {
                        if (tempArr.length < weeks[newI].length) {
                          ...
                          if (!isPush) {
                            tempArr.push(val); // console.log("next col ["+ (newI + 1) +"]["+ k +"] => " + val);
                          }
                        }
                      }
                    }
    ...
    */
    ...
    var draw = function draw() {
      ...
      var totalWeeks = weekCount(y, m + 1); //console.log("totalWeeks", totalWeeks);
      ...
      var sundayIndex = dayIndex == 0 ? 0 : 7 - dayIndex;
      var saturdayIndex = dayIndex == 0 ? 7 - 1 : 7 - (dayIndex + 1); //console.log("sundayIndex", sundayIndex);
        ...
        for (var j = 0; j < nDate; j++) {
          ...
          if (typeof settings.notes !== 'undefined') {
            if (dateIsNotes(new Date(year, month - 1, date))) {
              var note_types = getNote(new Date(year, month - 1, date)).note.join(' ');
              colDateClass += " note " + note_types;
            }
          }
    ...
    var getNote = function getNote(date) {
      var notesLength = settings.notes.length;
    ...
    return {
      ...
      returnNotes: function returnNotes() {
        return settings.notes;
      },
      addNoteExpanded: function addNoteExpanded(date) {
        var tripsitta_notes = this.returnNotes();
        var dateString = new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split("T")[0];
        var added = false;
        var removed = false;

        for (var i = 0; i < tripsitta_notes.length; i++) {
          if (tripsitta_notes[i].date === dateString) {
            if ($.inArray('babysitter-day', tripsitta_notes[i].note) > -1 && $.inArray('babysitter-night', tripsitta_notes[i].note) > -1) {
              tripsitta_notes[i].note.splice($.inArray('babysitter-day', tripsitta_notes[i].note), 1);
              added = true;
            } else if ($.inArray('babysitter-night', tripsitta_notes[i].note) > -1) {
              tripsitta_notes[i].note.splice($.inArray('babysitter-night', tripsitta_notes[i].note), 1);
              tripsitta_notes[i].note.push('babysitter-day');
              added = true;
            } else {
              tripsitta_notes[i].note.splice($.inArray('babysitter-day', tripsitta_notes[i].note), 1);

              if (tripsitta_notes[i].note.length === 0) {
                tripsitta_notes.splice(i, 1);
              }
        ...
        if (added !== true && removed !== true) {
          tripsitta_notes.push({
            "date": dateString,
            "note": ['babysitter-day', 'babysitter-night']
          });
        }

        this.update();
      },
      addNote: function addNote(date, new_note) {
        var tripsitta_notes = this.returnNotes();
        var dateString = new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split("T")[0];
        var added = false;
        var removed = false;
        ...
        for (var i = 0; i < tripsitta_notes.length; i++) {
          if (tripsitta_notes[i].date === dateString) {
            if ($.inArray(new_note, tripsitta_notes[i].note) <= -1) {
              tripsitta_notes[i].note.push(new_note);
              added = true;
            } else {
              tripsitta_notes[i].note.splice($.inArray(new_note, tripsitta_notes[i].note), 1);
...
/***/ "./resources/js/pages/babysitter-availability.js":
/*!*******************************************************!*\
  !*** ./resources/js/pages/babysitter-availability.js ***!
  \*******************************************************/
/*! no exports provided */
...
/* harmony import */ var _dncalendar__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../dncalendar */ "./resources/js/dncalendar.js");
/* harmony import */ var _dncalendar__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_dncalendar__WEBPACK_IMPORTED_MODULE_0__);
...
$(document).ready(function () {
  ...
  $('form').on('submit', function (e) {
    ...
    var input = $("<input>").attr("type", "hidden").attr("name", "availability_babysitter").val(JSON.stringify(my_calendar.returnNotes()));
    var input2 = $("<input>").attr("type", "hidden").attr("name", "availability_holiday_nanny").val(JSON.stringify(my_calendar2.returnNotes()));
...
/***/ 0:
/*!***********************************************************************************************************************!*\
  !*** multi ./resources/js/pages/babysitter-availability.js ./resources/sass/app.scss ./resources/sass/app-admin.scss ***!
  \***********************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

__webpack_require__(/*! D:\laragon\www\tripsitta\tripsitta\resources\js\pages\babysitter-availability.js */"./resources/js/pages/babysitter-availability.js");
__webpack_require__(/*! D:\laragon\www\tripsitta\tripsitta\resources\sass\app.scss */"./resources/sass/app.scss");
module.exports = __webpack_require__(/*! D:\laragon\www\tripsitta\tripsitta\resources\sass\app-admin.scss */"./resources/sass/app-admin.scss");
