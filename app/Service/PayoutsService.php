...
class PayoutsService
{
    ...
    public function __construct($profileId = null){
        ...
        $this->tw->profileId = config('tripsitta.transferwise.profile_id');
        if($profileId !== null)
        {
        ...
        $this->tw->api_key = config('tripsitta.transferwise.key');
        $this->tw->priv_pem = config('tripsitta.transferwise.private_pem');
    }
    ...
    public function getTransferById(string $transferId)
    {
        return $this->GET("/v1/transfers/$transferId");
    }

    public function getExchangeRate(string $source, string $target)
    {
        return $this->GET("/v1/rates?source=$source&target=$target");
    }

    /**
     * Provide a 3-char currency code as param. e.g. 'EUR'. If null, then return account balances for all currencies
     */
    ...
    private function curl($mode, $curl_url,$data=NULL,$headers=NULL){
        ...
        if(!empty($this->OTT)){
            ...
            if($SCA->status==403 && !empty($SCA->path)){
                $pkeyid = openssl_pkey_get_private('file://'.base_path().$this->tw->priv_pem);
